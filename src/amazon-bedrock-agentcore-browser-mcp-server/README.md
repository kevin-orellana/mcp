...
```bash
uvx awslabs.amazon-bedrock-agentcore-browser-mcp-server@latest
```

Or from a local source checkout (for development):

```bash
uv run --directory /path/to/amazon-bedrock-agentcore-browser-mcp-server \
...
```json
{
  "mcpServers": {
    "amazon-bedrock-agentcore-browser-mcp-server": {
      "command": "uvx",
      "args": ["awslabs.amazon-bedrock-agentcore-browser-mcp-server@latest"],
      "env": {
  ...
  "mcpServers": {
    "amazon-bedrock-agentcore-browser-mcp-server": {
      ...
      "args": [
        ...
        "run",
        "--from",
        "awslabs.amazon-bedrock-agentcore-browser-mcp-server@latest",
        "awslabs.amazon-bedrock-agentcore-browser-mcp-server.exe"
      ],
      "env": {
        "AWS_REGION": "us-east-1",
        "AWS_PROFILE": "your-profile"
...
Or using Docker after a successful `docker build -t mcp/bedrock-agentcore-browser .`:
  ...
  "mcpServers": {
    "amazon-bedrock-agentcore-browser-mcp-server": {
      ...
      "args": [
        ...
        "mcp/bedrock-agentcore-browser:latest"
      ],
      "env": {},
      "disabled": false,
      "autoApprove": []
...
>
> The agent calls `start_browser_session`, then `browser_navigate(url='http://example.com')`,
...
**Parameters:**
- `timeout_seconds`: Session timeout in seconds (default: 300)
- `region`: AWS region override (default: from environment)
- `profile_name`: AWS profile override (default: from environment)

**Returns:** Session ID, status, automation stream URL, viewport dimensions.
...
```python
browser_click(session_id: str, ref: str) -> str
```

#### browser_type

Type text into an element by ref. Optionally clear existing content first or submit with Enter.
